...
@admin_definitions_bp.route('/module-task-logs/<int:task_log_id>/status', methods=['PUT'])
def update_module_task_log_status_route(task_log_id):
    data = request.get_json()
    if not data or 'status' not in data:
        return jsonify(error="Missing 'status' in request data"), 400

    new_status = data['status']
    station_finish = data.get('station_finish')
    notes = data.get('notes')

    # Validate status
    allowed_statuses = ['In Progress', 'Completed', 'Paused'] # 'Not Started' is initial, not set via update
...
@admin_definitions_bp.route('/panel-task-logs/<int:panel_task_log_id>/status', methods=['PUT'])
def update_panel_task_log_status_route(panel_task_log_id):
    data = request.get_json()
    if not data or 'status' not in data:
        return jsonify(error="Missing 'status' in request data"), 400
    ...
    allowed_statuses = ['In Progress', 'Completed', 'Paused']
    if new_status not in allowed_statuses:
        return jsonify(error=f"Invalid status. Must be one of: {', '.join(allowed_statuses)}"), 400
